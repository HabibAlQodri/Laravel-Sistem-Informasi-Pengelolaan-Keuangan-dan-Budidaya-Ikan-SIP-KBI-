...
<body class="bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 transition duration-500">
                        ...
                        <table class="w-full">
                            <thead class="bg-sipkbi-green text-white">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-semibold uppercase">Nama Pakan</th>
                                    <th class="px-6 py-3 text-left text-xs font-semibold uppercase">Jenis Pakan</th>
                                    <th class="px-6 py-3 text-left text-xs font-semibold uppercase">Harga/Kg</th>
                                    <th class="px-6 py-3 text-left text-xs font-semibold uppercase">Stok (Kg)</th>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                    <div class="p-6">
                        <div class="flex justify-between items-center mb-6">
                            <h2 id="modal-title" class="text-2xl font-bold">Tambah Pakan</h2>
                        ...
                        <form id="pakan-form" class="space-y-4">
                            <input type="hidden" id="id">

                            <div>
                                <label class="block text-sm font-medium mb-2">Nama Pakan</label>
                                <input type="text" id="nama_pakan" required class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 focus:ring-2 focus:ring-sipkbi-green focus:border-transparent">
                            </div>

                            <div>
                                <label class="block text-sm font-medium mb-2">Jenis Pakan</label>
                                <input type="text" id="jenis_pakan" required class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 focus:ring-2 focus:ring-sipkbi-green focus:border-transparent">
                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label class="block text-sm font-medium mb-2">Stok (Kg)</label>
                                    <input type="number" step="0.01" id="stok_kg" required class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 focus:ring-2 focus:ring-sipkbi-green focus:border-transparent">
                                </div>
                            </div>

                            <div>
                                <label class="block text-sm font-medium mb-2">Supplier</label>
                                <input type="text" id="supplier" required class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 focus:ring-2 focus:ring-sipkbi-green focus:border-transparent">
                            </div>
    ...
    <script>
        ...
        const pakanForm = document.getElementById('pakan-form');

        let isEditMode = false;

        function openModal(mode, data = null) {
            isEditMode = mode === 'edit';
            modalTitle.textContent = isEditMode ? 'Edit Pakan' : 'Tambah Pakan';

            if (isEditMode && data) {
                document.getElementById('id').value = data.id;
                document.getElementById('nama_pakan').value = data.nama_pakan;
                document.getElementById('jenis_pakan').value = data.jenis_pakan;
                document.getElementById('harga_per_kg').value = data.harga_per_kg;
                document.getElementById('stok_kg').value = data.stok_kg;
        ...
        async function loadPakan() {
            try {
                ...
                showAlert('Gagal memuat data pakan', 'error');
            }
        }
        ...
        function renderTable(data) {
            ...
            tbody.innerHTML = data.map((item, index) => `
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                    ...
                    <td class="px-6 py-4 text-sm font-medium">${item.nama_pakan}</td>
                    <td class="px-6 py-4 text-sm">${item.jenis_pakan}</td>
                    <td class="px-6 py-4 text-sm">Rp ${parseFloat(item.harga_per_kg).toLocaleString('id-ID')}</td>
                    <td class="px-6 py-4 text-sm">${parseFloat(item.stok_kg).toLocaleString('id-ID')}</td>
                    <td class="px-6 py-4 text-sm">${item.supplier}</td>
                    <td class="px-6 py-4 text-center">
                        <div class="flex justify-center space-x-2">
                            <button onclick='editPakan(${JSON.stringify(item)})' class="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300" title="Edit">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                </svg>
                            </button>
                            <button onclick="deletePakan(${item.id})" class="text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300" title="Hapus">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        pakanForm.addEventListener('submit', async (e) => {
            ...
            const formData = {
                nama_pakan: document.getElementById('nama_pakan').value,
                jenis_pakan: document.getElementById('jenis_pakan').value,
                harga_per_kg: document.getElementById('harga_per_kg').value,
                stok_kg: document.getElementById('stok_kg').value,
        ...
        async function deletePakan(id) {
            ...
            try {
                const response = await fetch(`/api/pakan/${id}`, {
                    method: 'DELETE',
                    headers: {
                        'Accept': 'application/json',
                        'Content-Type': 'application/json',
